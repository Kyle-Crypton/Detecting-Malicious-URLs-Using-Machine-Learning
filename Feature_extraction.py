...
def find_ele_with_attribute(dom, ele, attribute):
	"""docstring for find_ele_with_attribute"""
...
def Security_sensitive(tokens_words):
	"""docstring for Security_sensitive"""
...
def web_content_features(url):
	"""docstring for web_content_features"""
	wfeatures = {}
	total_cnt = 0
	try:
...
def safebrowsing(url):
	...
	try:
		...
		if res.code == 204:
			...
			# print 'The required URL is either phishing, malware or both, see the response body for the specific type.'
			return 1
		elif res.code == 204:
...
def feature_extract(url_input):
	"""docstring for feature_extract"""
	Feature = {}
	# Extract bag of words stings delimited by (.,/,?,,=,-,_)
